...
- ECMA2015: `Object.getPrototypeOf()`，对应的是 `Object.setPrototypeOf()`
...
函数的 `func.prototype` 表示的是当此函数作为构造器时，构造的对象对应的prototype链接：
...
```js
// 直接构造的对象原型链: o1 ---> Object.prototype ---> null
let o1 = {
    a: 1
}
...
- 若不想从原型链往上traverse，可以调用 `obj.hasOwnProperty()`
...
| 创建方式 | 原型链 |
|----------|--------|
| 字面量 `{}` | obj → Object.prototype → null |
| 构造函数 `new` | obj → Constructor.prototype → Object.prototype → null |
| `Object.create()` | obj → 传入的对象 → ... |
| `class` | obj → Class.prototype → ParentClass.prototype → ... |
